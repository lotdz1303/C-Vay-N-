...
const CELL = 52;
const MARGIN = 42;
const BOARD_PIXELS = MARGIN * 2 + CELL * (BOARD_SIZE - 1);
...
function drawBoard(board) {
    ...
    roundRect(ctx, 0, 0, BOARD_PIXELS, BOARD_PIXELS, 18, true, false);

    ctx.lineWidth = 14;
    ctx.strokeStyle = "#7c4a12";
    roundRect(ctx, 8, 8, BOARD_PIXELS - 16, BOARD_PIXELS - 16, 10, false, true);
    ...
    for (let i = 0; i < BOARD_SIZE; i++) {
        ...
        ctx.lineTo(MARGIN + CELL * (BOARD_SIZE - 1), pos);
        ctx.stroke();

        ctx.beginPath();
        ctx.moveTo(pos, MARGIN);
        ctx.lineTo(pos, MARGIN + CELL * (BOARD_SIZE - 1));
        ctx.stroke();
    }
    ...
    for (let i = 0; i < BOARD_SIZE; i++) {
        for (let j = 0; j < BOARD_SIZE; j++) {
            ...
            if (cell === "X") {
                drawStone(cx, cy, "#111111", "#000000");
            } else if (cell === "O") {
                drawStone(cx, cy, "#f8fafc", "#9ca3af");
            }
        }
    }
}
...
function getClickedPosition(event) {
    ...
    for (let row = 0; row < BOARD_SIZE; row++) {
        for (let col = 0; col < BOARD_SIZE; col++) {
            const pointX = MARGIN + col * CELL;
            const pointY = MARGIN + row * CELL;
...
function launchConfetti() {
    ...
    const colors = ["#22c55e", "#facc15", "#38bdf8", "#f97316"];
    ...
    function frame() {
        ...
        confetti.style.animation = "fall 0.9s linear forwards";
